        ...
        try {
            ...
            if (cameras && cameras.length > 0) {
                ...
                cameras.forEach((camera, index) => {
                    ...
                    if (camera.label && (camera.label.toLowerCase().includes('front') || camera.label.toLowerCase().includes('depan') || camera.label.toLowerCase().includes('user'))) {
                        frontCameraIndex = index;
                    }
                });
                
                cameraSelect.selectedIndex = frontCameraIndex !== -1 ? frontCameraIndex : 0;
            } else {
                cameraSelect.innerHTML = '<option value="">Tidak ada kamera tersedia</option>';
                alert('Tidak ada kamera terdeteksi. Pastikan kamera terhubung dan browser memiliki izin.');
            }
        } catch (err) {
